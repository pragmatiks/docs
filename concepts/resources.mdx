...
| State | Description |
|-------|-------------|
| `draft` | Created but not yet queued for processing (use `--draft` flag) |
| `pending` | Queued for provider to handle (default behavior) |
...
<Note>
Updates and deletes reuse the same state flow: the resource transitions from `ready` → `pending` → `processing` → `ready` (or `failed`).
</Note>
